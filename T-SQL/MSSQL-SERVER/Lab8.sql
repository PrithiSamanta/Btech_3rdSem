...
SELECT MAX(SALARY) AS RAJKOT_MAX_SAL
FROM EMPLOYEE
WHERE CITY = 'RAJKOT'
...
SELECT COUNT(DISTINCT CITY) AS UNIQ_CITY
FROM EMPLOYEE

-- 10. Count unique departments. 
SELECT COUNT(DISTINCT DEPARTMENT) AS UNIQ_DEPARTMENT
FROM EMPLOYEE
...
SELECT COUNT(EID) AS MALE_EMPLOYEE
FROM EMPLOYEE
WHERE GENDER='MALE'
...
SELECT CITY, COUNT(EID) AS MALE_EMPLOYEE
FROM EMPLOYEE
WHERE GENDER='MALE'
GROUP BY CITY

-- 20. Display maximum salary in each department where city is not Ahmedabad. 
SELECT DEPARTMENT, MAX(SALARY) AS MAXIMUM_SAL
FROM EMPLOYEE
WHERE CITY !='AHMEDBAD'
GROUP BY DEPARTMENT
...
SELECT CITY, MIN(SALARY) AS MIN_FEMALE_SAL
FROM EMPLOYEE
WHERE GENDER='FEMALE'
GROUP BY CITY
