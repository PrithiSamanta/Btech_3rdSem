...
SELECT SNAME
FROM STUDENT S
WHERE SPI > ANY (SELECT AVG(SPI) FROM STUDENT GROUP BY BRANCH)

--12. Display the branch having maximum average SPI. 
SELECT BRANCH,AVG(SPI)
FROM STUDENT
GROUP BY BRANCH
HAVING AVG(SPI) >= ALL (SELECT AVG(SPI) A_SPI FROM STUDENT GROUP BY BRANCH)
...
JOIN RESULT R
ON S.RNO = R.RNO
WHERE SPI >ALL (SELECT SPI FROM STUDENT_INFO S JOIN RESULT R ON S.RNO=R.RNO WHERE BRANCH='ME') 
...
JOIN RESULT R
ON S.RNO = R.RNO
WHERE SPI <ANY (SELECT SPI FROM STUDENT_INFO S JOIN RESULT R ON S.RNO=R.RNO WHERE BRANCH='ME') 
