...
SELECT *
FROM EMPLOYEE
WHERE CITY LIKE '%RA%' AND SALARY <13000 AND JOININGYEAAR LIKE '%6'

-- 22. Display employees whose SALARY between 10000 and 15000 and CITY name contains 'KO' and 
-- FIRSTNAME start with H. 
SELECT *
FROM EMPLOYEE
WHERE CAST(SALARY AS VARCHAR(30)) LIKE '[10000-15000]%' AND CITY LIKE '%KO%' AND FIRSTNAME LIKE 'H%'
...
SELECT *
FROM EMPLOYEE
WHERE CITY LIKE '%' AND NOT FIRSTNAME LIKE '[AEIOU]%' AND NOT DEPARTMENT IN ('HR','IT')

-- 30. Display employees whose CITY is not NULL, FIRSTNAME ends with vowels, and DEPARTMENT is neither 
-- ‘HR’ nor ‘IT’. 
SELECT *
FROM EMPLOYEE
WHERE CITY LIKE '%' AND FIRSTNAME LIKE '%[AEIOU]' AND NOT DEPARTMENT IN ('HR','IT'); 